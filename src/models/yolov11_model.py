"""YOLOv11 Lightning Module - Ultralytics YOLO 래퍼"""
import os
import yaml
...
class YOLOv11LightningModule(pl.LightningModule):
    """YOLOv11 Lightning Module (Ultralytics YOLO 래퍼)"""
    
    def __init__(
        self,
        model_size: str = "m",  # n, s, m, l, x
        pretrained_path: str = "yolo11m.pt",
        num_classes: int = 3,
        lr0: float = 0.001,
        lrf: float = 0.01,
    ...
    def build_train_args(self, trainer_config: Dict[str, Any], data_config: Dict[str, Any]):
        ...
        train_args = {
            ...
            # 체크포인트 저장
            'save_period': self.training_config.get('save_period', 1),    # 매 에포크 평가
            'save': self.training_config.get('save', True),              # 저장 활성화
            'save_best': self.training_config.get('save_best', True),    # 최고 성능 저장
            'save_last': self.training_config.get('save_last', True),    # 마지막 저장
        }
    ...
    def fit(self, trainer_config: Dict[str, Any], data_config: Dict[str, Any]):
        ...
        print("=" * 60)
        print(f"Model: YOLOv11 ({self.hparams.model_size})")
...
class YOLOv11Wrapper:
    ...
    def train(self):
        ...
        print(f"Model: {self.model_config.get('arch_name', 'yolov11')} ({self.model_config.get('model_size', 'm')})")
