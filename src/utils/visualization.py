    ...
    for idx, result in enumerate(yolo_results[:total_images]):
        ...
        if yolo_output_dir is not None and hasattr(result, 'path'):
            ...
            if yolo_save_path.exists():
                # yolo_predictions용 이미지 생성 (클래스별 색상)
                ...
                # yolo_predictions 폴더에 저장 (원본 품질 보존)
